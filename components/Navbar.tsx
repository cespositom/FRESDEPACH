...
export default function Navbar({ perfil }: { perfil: Perfil }) {
  ...
  const links = [
    ...
    { href: '/quotesimport',     label: 'Cotizar Imp.' },
    ...(perfil.perfil === 'admin' ? [{ href: '/admin/usuarios', label: 'Usuarios' }] : []),
    ...(perfil.perfil === 'admin' ? [{ href: '/admin/proveedores', label: 'Proveedores' }] : []),
    ...(perfil.perfil === 'admin' ? [{ href: '/admin/configuracion', label: 'Config' }] : []),
  ]
          ...
          <div className="hidden md:flex gap-0.5">
            {links.map(l => (
              <Link key={l.href} href={l.href}
                className={`px-2.5 py-1 rounded-lg text-xs transition uppercase tracking-wide ${
        ...
        <div className="md:hidden border-t border-gray-100 px-4 py-3 space-y-1">
          {links.map(l => (
            <Link key={l.href} href={l.href}
              ...
              className={`block px-3 py-1.5 rounded-lg text-xs transition uppercase tracking-wide ${
                pathname.startsWith(l.href)
                  ? 'bg-blue-50 text-blue-600 font-bold'
                  : 'text-gray-600 font-medium hover:bg-gray-100'
              }`}>
